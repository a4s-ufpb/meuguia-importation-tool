...
class CsvImporterGUI(ctk.CTk):
    ...
    def display_authentication(self):
        ...
        user_label.grid(row=1, column=0, pady=(10, 5), padx=(10, 5), sticky='e')

        user_entry = ctk.CTkEntry(self.auth_frame)
        user_entry.grid(row=1, column=1, columnspan=2, pady=(10, 5), padx=(5, 10), sticky='ew')

        password_label = ctk.CTkLabel(self.auth_frame, text='Senha:')
        password_label.grid(row=2, column=0, pady=(5, 10), padx=(10, 5), sticky='e')

        password_entry = ctk.CTkEntry(self.auth_frame, show='*')
        password_entry.grid(row=2, column=1, columnspan=2, pady=(5, 10), padx=(5, 10), sticky='ew')

        submit_button = ctk.CTkButton(self.auth_frame, text='login', command=lambda: self._login(user_entry.get(), password_entry.get()))
        submit_button.grid(row=3, column=2, pady=(10, 10), padx=(10, 10), sticky='e')
    ...
    def display_select_file(self):
        ...
        import_button.grid(row=0, column=0, pady=20, padx=20)
    ...
    def display_attractions(self):
        ...
        self.import_button.grid(row=1, column=0, pady=(8, 20), padx=32, stick='nsew')
